  ...
  <div class="container" style="margin-top: 60px;">
    <form clas="form-container" @submit.prevent="handleLogin">
      ...
      <div class="form-floating">
        <input v-model="username" type="username" class="form-control" id="floatingInput" required>
        <label for="floatingInput">Username</label>
      </div>
      <div class="form-floating">
        <input v-model="password" type="password" class="form-control" id="floatingPassword" required>
  ...
  navigator.geolocation.getCurrentPosition(async (position) => {
    try {
      ...
      try {
        ...
        if (tareasPorVencer.data && tareasPorVencer.data.length > 0) {
          // Aquí puedes usar un modal, toast, o alert personalizado
          alert(`¡Tienes ${tareasPorVencer.data.length} tareas próximas a vencer!`);
          // O mejor aún, muestra un modal bonito en vez de alert
        }
      } catch (e) {
